...
foreach ($arResult['ITEMS'] as $key => $item) {
	...
	$arResult['ITEMS'][$key]['CLINICS'] = $item['PROPERTY_84'];
	$arResult['ITEMS'][$key]['FOR_CHILD'] = $item['PROPERTIES']['PG_FOR_CHILD']['VALUE'];
}
...
$_sections = CIBlockSection::GetList(array('SORT' => 'ASC'),array('IBLOCK_ID' => 10,'GLOBAL_ACTIVE'=>'Y'));
$arResult['SECTIONS'] = array();
while ($sections = $_sections->GetNext()) {
	$arUF = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("IBLOCK_".$arResult["ID"]."_SECTION",$sections['ID']);

	$arResult['SECTIONS'][$sections['ID']]['NAME'] = $sections['NAME'];
	$arResult['SECTIONS'][$sections['ID']]['PREVIEW_TEXT'] = $arUF['UF_PG_PREVIEW_TEXT']['VALUE'];
	$arResult['SECTIONS'][$sections['ID']]['URL'] = $sections['SECTION_PAGE_URL'];		
	$arResult['SECTIONS'][$sections['ID']]['DESCRIPTION'] = $sections['DESCRIPTION'];		
	$arResult['SECTIONS'][$sections['ID']]['IMG'] = CFile::GetPath($sections['PICTURE']) ;	  
} 
